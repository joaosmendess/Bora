...
function DaysDisplay({ days }: { days: number | null }) {
  ...
  if (days > 0) {
    const color = days <= 60 ? '#E8714C' : '#2FA39A'
    return (
  ...
  if (days === 0) return (
    <span style={{ fontFamily: 'var(--font-bricolage), sans-serif', fontWeight: 800, fontSize: 24, color: '#E8714C' }}>
      hoje!
    </span>
  )
  return (
    <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'var(--font-bricolage), sans-serif', fontWeight: 800, fontSize: 22, color: '#7FA86B' }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        background: '#fff', border: '1px solid #EFE6D7', borderRadius: 14,
    ...
    >
      ...
      <div style={{ minWidth: 100, flexShrink: 0 }}>
        ...
        {d.target_date && (
          <p style={{ fontSize: 12, color: '#8A8178', marginTop: 2 }}>
            {new Date(d.target_date).toLocaleDateString('pt-BR', { month: 'short', year: 'numeric' })}
          </p>
        )}
      </div>

      <div style={{ width: 1, height: 40, background: '#EFE6D7', flexShrink: 0 }} />

      {/* Mini thumb */}
      <div
        style={{
          width: 48, height: 48, borderRadius: 10, flexShrink: 0,
          backgroundImage: (d.cover_photo && d.cover_photo.startsWith('http')) ? `url(${d.cover_photo})` : (d.cover_photo || 'linear-gradient(135deg, #E8714C, #E8924C)'),
          backgroundSize: 'cover', backgroundPosition: 'center',
        }}
      />

      <div style={{ flex: 1, minWidth: 0 }}>
        <h3 style={{ fontFamily: 'var(--font-bricolage), sans-serif', fontWeight: 700, fontSize: 17, color: '#2B2622', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {d.name}
        </h3>
        <p style={{ fontSize: 13, color: '#8A8178' }}>{d.country}</p>
      </div>
    ...
    >
      ...
      {upcoming.length > 0 && (
        <section style={{ marginBottom: 36 }}>
          <h2 style={{ fontSize: 13, fontWeight: 700, color: '#8A8178', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 14 }}>
            Próximas viagens
          </h2>
      ...
      {upcoming.length === 0 && visited.length === 0 && (
        <div style={{ textAlign: 'center', padding: '80px 20px', color: '#8A8178' }}>
          <Calendar size={40} style={{ color: '#EBE1D2', margin: '0 auto 16px' }} />
          <p style={{ fontSize: 16, fontWeight: 600, color: '#2B2622', marginBottom: 6 }}>
          ...
          <button
            ...
            style={{
              ...
              padding: '9px 18px', fontSize: 13, fontWeight: 600, color: '#2B2622',
              cursor: 'pointer',
            }}
          >
            <ArrowLeft size={14} />
      ...
      {noDateCount > 0 && (upcoming.length > 0 || visited.length > 0) && (
        <p style={{ fontSize: 13, color: '#B0917A', marginTop: 24, textAlign: 'center' }}>
